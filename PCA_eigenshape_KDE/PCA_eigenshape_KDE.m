...
stiff_idx = f_coeffs(:,end-1)>0;
soft_idx = f_coeffs(:,end-1)<1;

% identifiers for unactivated and activated
act_idx = f_coeffs(:,end)>0;
unact_idx = f_coeffs(:,end)<1;

% removes soft/stiff identifer and unactivated/activated identifer
f_coeffs = f_coeffs(:,1:end-2);
...
for k = 1:3 % iterates for PC axes 1-3
    ...
    % kernel density estimation of PC scores between soft and stiff
    figure (600+k)
    [f_unact,xi_unact] = ksdensity(score(unact_idx,k),pts); % KDE for soft
    plot(xi_unact,f_unact,'color',[0.8, 0, 0.8],'LineWidth',16); hold on; % Plot for soft
    [f_act,xi_act] = ksdensity(score(act_idx,k),pts); % KDE for stiff
    plot(xi_act,f_act,'color',[0, 0.8, 0],'LineWidth',16); hold off; % Plot for stiff
    xlabel(strcat("PC",num2str(k)),'FontWeight','bold')
    ylabel("PDE",'FontWeight','bold')
    set(get(gca, 'XAxis'), 'FontWeight', 'bold');
    set(get(gca, 'YAxis'), 'FontWeight', 'bold');
    pbaspect([1 1 1])
    exportgraphics(gcf,strcat('UNACT_and_ACT_KDE_PCA_',num2str(k),'.png'),'Resolution',300)
